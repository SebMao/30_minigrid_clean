...
class Actor(nn.Module):
    def __init__(self):
        ...
        self.conv_local = Conv2d(1, 5, 3, padding=1)
        self.fc2 = layer_init(nn.Linear(125, 25), std=0.01)
        self.fc3 = layer_init(nn.Linear(25, 3), std=0.01)

        self.test_fc = layer_init(nn.Linear(25, 25), std=0.1)
        self.test_fc2 = layer_init(nn.Linear(25, 3), std=0.1)
    ...
    def forward(self, s0, si, action=None):
        ...
        # si_out = F.relu(self.fc2(si_1.view(bs, 125)))

        # tmp
        # si_out = F.relu(self.fc3(si_out))
        si = si.view(bs, 25)
        si_1 = F.tanh(self.test_fc(si))
        si_out = F.tanh(self.test_fc2(si_1))
        logits = si_out
...
def main(args):
    ...
    local_obses = torch.zeros(args.num_steps, 1, env.view_size, env.view_size)

    actions = torch.zeros(args.num_steps, 1)
    ...
    for update in tqdm(range(1, num_updates + 1)):
        ...
        for step in range(0, args.num_steps):
            ...
            maps[step] = next_map
            local_obses[step, 0] = next_local_obs[0]
            dones[step] = next_done

            # ALGO LOGIC: action logic
            # with torch.no_grad():
            with torch.no_grad():
                action, logprob, logits = actor(maps[step].unsqueeze(0).unsqueeze(0),
                                            local_obses[step, 0].unsqueeze(0).unsqueeze(0))
            ...
            _, reward, done, _ = env.step([actions[step][0], 4, 4])
            next_map = env.get_s0()
            next_local_obs = env.get_s_agent()[0].unsqueeze(0)  # 1, 5, 5

            # if done[0] == 1:
            #     with torch.no_grad():
            #         value_ = agent.get_value(info)
            #         reward = reward + args.gamma * value_
            rewards[step] = torch.tensor(reward[0])
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                newlogprob = torch.zeros(args.minibatch_size, 1)  # BS * #agents
                mb_inds = b_inds[start:end]
                for i in range(1):
                    _, newlogprob[:, i], _ = actor(maps[mb_inds].unsqueeze(1), local_obses[mb_inds, i].unsqueeze(1), actions[mb_inds, i].unsqueeze(1))
                newvalue = critic(maps[mb_inds].unsqueeze(1), local_obses[mb_inds].unsqueeze(2))
        ...
        y_pred, y_true = values.cpu().numpy(), b_returns.cpu().numpy()
        var_y = np.var(y_true)
