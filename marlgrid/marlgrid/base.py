...
class MultiGrid:
    ...
    def __init__(self, shape, obj_reg=None, orientation=0):
        ...
        if isinstance(shape, tuple):
            ...
            self.grid_mask = np.zeros((self.width, self.height), dtype=np.uint8)  # w,h
    ...
    def render(self, tile_size, highlight_mask=None, visible_mask=None, top_agent=None):
        ...
        for j in range(0, self.height):
            for i in range(0, self.width):
                ...
                # if tile_size == 32:
                #     alpha = 200
                #     tile_img[0], tile_img[-1], tile_img[:,0], tile_img[:,-1] = alpha, alpha, alpha, alpha
                # else:
                #     alpha = 20
        ...
        if highlight_mask is not None:
            ...
            # hm = np.kron(highlight_mask.T, np.full((tile_size, tile_size), 1, dtype=np.uint16)
            #     )[...,None] # arcane magic.
            # img = img * hm + 50 * hm
...
class MultiGridEnv(gym.Env):
    def __init__(
        ...
        self.viewed_area = np.zeros((self.width, self.height), dtype=bool)
    ...
    def reset(self, **kwargs):
        ...
        for agent in self.agents:
            if agent.spawn_delay == 0:
                ...
                agent.activate()
            self.viewed_area[agent.pos[0], agent.pos[1]] = True
    ...
    def step(self, actions):
        ...
        step_rewards = np.zeros((len(self.agents,)), dtype=float) + R_idle
        ...
        for shuffled_ix in iter_order:
            ...
            if agent.active:
                ...
                # Move forward
                    ...
                    if isinstance(fwd_cell, (Lava, Goal)):  # 找到目标
                        self.grid.set(*fwd_pos, agent)
                        self.grid.set(*cur_pos, None)
                        agent.pos = fwd_pos
                        step_rewards[agent_no] += R_r
                        # for a in iter_order:
                        #     step_rewards[a] += R_r
                    ...
                    if fwd_cell is None:
                        ...
                        self.grid.set(*cur_pos, None)
                        agent.pos = fwd_pos
                    if isinstance(fwd_cell, (Wall, GridAgent)):  # 碰撞
                        # print("collision, agent_no:", agent_no)
                        step_rewards[agent_no] += R_c
                    if fwd_cell == agent:
                        self.grid.set(*cur_pos, None)
    ...
    def render(
        ...
        # Render the whole grid  # grad
        # img = self.grid.render(tile_size, highlight_mask=highlight_mask if highlight else None)
        img = self.grid.render(tile_size, highlight_mask=self.viewed_area)
        # img = self.grid.render(tile_size)

        self.viewed_area = np.logical_or(self.viewed_area, highlight_mask)
